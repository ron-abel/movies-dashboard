...
export default function AddMovies() {
  ...
  const AddMovie = async () => {
    if (isFormValid) {
      await axios.post("http://localhost:3000/api/movies", { title: title, poster: poster, year: year })
      router.push('/movies');
  ...
  return (
    ...
    <>
      <div className="add-movies d-flex align-items-center container">
        <div className="w-100"><h2 className=" text-white">Create a new movie</h2>
              ...
              <div className="card border-0 bg-transparent movie-form rounded-0 ">
                <form className="bg-transparent ">
                  ...
                  <div className="d-flex action-buttons justify-sm-content-between gap-3">
                    ...
                    <button onClick={AddMovie} type="button" className="btn  border-0 text-white">Submit</button>
                  </div>
                </form>
              </div>
            </div>
